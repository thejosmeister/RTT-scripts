        ...
        if trip.find('Activities') is not None:
            ...
            for activity in trip.find('Activities').findall('Activity'):
                if activity.find('AssociatedUID') is not None:
                    location[ACT_DICT[activity.find('Activity').text]] = activity.find('AssociatedUID').text
                else:
                    location[ACT_DICT[activity.find('Activity').text]] = None

        out.append(location)
    return out


def parse_individual_xml_tt(xml_tt: ET.Element, tiploc_dict: dict) -> dict:
    json_tt = {}
    first_loc_is_stop = False
...
def parse_full_xml_tt(file: str, locations_file: str, overwrite_existing: bool):
    ...
    for tt in list_of_tt_elts:
        if overwrite_existing is True:
            tt_db.add_tt(parse_individual_xml_tt(tt, tiploc_dict))
        else:
            tt_db.add_tt_if_not_present(parse_individual_xml_tt(tt, tiploc_dict))
...
parse_full_xml_tt('SavedTimetable.xml', 'sim_location_files/swindon_locations.txt')
